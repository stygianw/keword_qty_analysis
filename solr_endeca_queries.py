...
def create_solr_url(search_word):
    return "{}&{}".format(SOLR_QUERY, create_solr_q_param(search_word))
...
def find_solr_props(search_word):
    r = request.urlopen(create_solr_url(search_word))
    doc = r.read()
    root = ET.fromstring(doc)
...
def create_solr_search_string(solr_query):
...
def test_create_solr_query():
    assert create_solr_search_string('BEAUTY AND THE BEAST') == '+BEAUTY +AND +THE +BEAST "BEAUTY AND THE BEAST"'
    assert create_solr_search_string('FAIRIES') == "FAIRIES"


def create_solr_q_param(search_string):
    return parse.urlencode( {'q': "+({})".format(create_solr_search_string(search_string))} , safe="()\"")


def test_create_solr_q_param():
    assert create_solr_q_param("BEAUTY AND THE BEAST") == "q=%2B(%2BBEAUTY+%2BAND+%2BTHE+%2BBEAST+\"BEAUTY+AND+THE+BEAST\")"
